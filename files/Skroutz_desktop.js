...
async function tutorial() {
	try {
		const URL = 'https://www.skroutz.gr/c/891/branded-desktop-pc.html?price_drops=1'
		...
		while (currentPage <= pagesToScrape) {
			...
			let newResults = await config.page.evaluate(() => {
				let results = []
				let items = document.querySelectorAll('li.cf.card')
				items.forEach((item) => {
					try     //an brei antikeimena pou exoun eksantlithei tote phgaine catch
					{
						results.push({
							...
							image: item.querySelector('li.cf.card a.js-sku-link.pic img').src,
			...
			if (currentPage < pagesToScrape) {
				await config.page.goto('https://www.skroutz.gr/c/891/branded-desktop-pc.html?price_drops=1&page=' + (currentPage + 1), {
