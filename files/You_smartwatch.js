...
async function tutorial() {
	try {
		const URL = 'https://www.you.gr/smart-tech-gadgets/smartwatches-wearables/smartwatches?pagesize=60&view=grid'
				...
				items.forEach((item) => {
					try     //an brei antikeimena pou exoun eksantlithei tote phgaine catch
					{
						results.push({
							...
							image: getComputedStyle(item.querySelector('a.img_area.lazy')).getPropertyValue('background-image').slice(5).slice(0, -2),
							//image: item.querySelector('div.inner.radiusCorners').innerHTML,
