...
async function tutorial() {
	try {
		const URL = 'https://www.you.gr/ypologistes-perifereiaka/desktops/ola-ta-desktops?ps=80&view=grid'
		...
		while (currentPage <= pagesToScrape) {
			...
			if (currentPage < pagesToScrape) {
				...
				//await config.page.click('#pagination2 > li:nth-child('+(currentPage+1)+')')
				//await config.page.waitForSelector('div.cmnProductItem.flexCol_xs_6.flexCol_sm_6.flexCol_md_6.flexCol_lg_4.flexCol_xl_3')
				//await config.page.waitForSelector('#pagination2 > li:nth-child('+(currentPage+1)+')')

				//await config.page.waitForTimeout(5000)
				// (await config.page.click('a[data-page="2"]'))[1]
				// //await config.page.waitForTimeout(5000)
				// await config.page.waitForSelector('div.cmnProductItem.flexCol_xs_6.flexCol_sm_6.flexCol_md_6.flexCol_lg_4.flexCol_xl_3')
				// (await config.page.waitForSelector('a[data-page="2"]'))[1]
